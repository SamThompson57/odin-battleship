...
1) Begin your app by creating the Ship factory function.
    ...
    a) Your ‘ships’ will be objects that include their length, the number 
    of times they’ve been hit and whether or not they’ve been sunk.
   
    b) REMEMBER you only have to test your object’s public interface. 
    Only methods or properties that are used outside of your ‘ship’ 
    object need unit tests.
    
    c) Ships should have a hit() function that increases the number of 
    ‘hits’ in your ship.

    d) isSunk() should be a function that calculates it based on their 
    length and the number of ‘hits’.
...
2) Create Gameboard factory.
    ...
    b) Gameboards should be able to place ships at specific coordinates 
    by calling the ship factory function.

    c) Gameboards should have a receiveAttack function that takes a pair 
    of coordinates, determines whether or not the attack hit a ship and 
    then sends the ‘hit’ function to the correct ship, or records the 
    coordinates of the missed shot.

    d) Gameboards should keep track of missed attacks so they can display 
    them properly.

    e) Gameboards should be able to report whether or not all of their 
...
3) Create Player.
    ...
    b) The game is played against the computer, so make ‘computer’ 
    players capable of making random plays. The AI does not have to be 
    smart, but it should know whether or not a given move is legal. (i.e. 
...
*********
    ...
    coordinates. You can implement a system for allowing players to place 
    their ships later.
